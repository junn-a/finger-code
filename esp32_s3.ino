...
// ================= PIN MAP UNTUK ESP32-S3 =================
// Catatan pemilihan pin (khusus ESP32-S3 DevKitC-1 N8R8/N16R8):
// - Hindari GPIO 19/20 (dipakai USB D-/D+)
// - Hindari GPIO 26-32 & 33-37 (dipakai Flash/PSRAM pada modul R8/R16 tertentu)
// - Hindari GPIO 0/3/45/46 (strapping pin boot)
// - Hindari GPIO 43/44 (default UART0 / port USB-Serial programming)
// Karena ESP32-S3 pakai GPIO matrix, UART & I2C bisa dipetakan ke pin GPIO
// bebas mana pun yang aman di atas — jadi tinggal ganti nomor pin, konsep/
// state machine/HTTP handler semua TETAP SAMA seperti versi ESP32 asal.

// ---------- Relay ----------
#define RELAY_PIN 4   // GPIO4, aman & bebas dari flash/PSRAM/strap
const unsigned long RELAY_ON_DURATION = 1000; // relay nyala lalu mati otomatis

unsigned long relayOnAt = 0;
bool relayIsOn = false;

// ---------- Fingerprint sensor (UART via GPIO matrix) ----------
#define FP_RX_PIN 17  // ESP32-S3 RX <- TX sensor
#define FP_TX_PIN 18  // ESP32-S3 TX -> RX sensor

// ---------- I2C LCD ----------
#define I2C_SDA_PIN 8   // default I2C SDA di banyak devkit S3
#define I2C_SCL_PIN 9   // default I2C SCL di banyak devkit S3

// ---------- WiFi AP ----------
const char* AP_SSID = "Fingerprint-ESP32S3";
...
// ---------- Sensor ----------
HardwareSerial mySerial(2); // UART2, dipetakan ke FP_RX_PIN/FP_TX_PIN lewat GPIO matrix
