...
// ---------- WiFi AP (untuk akses admin lokal) ----------
const char* AP_SSID = "Fingerprint-ESP32";
const char* AP_PASS = "12345678"; // min 8 karakter

// ---------- WiFi Pabrik (STA, untuk kirim history) ----------
const char* STA_SSID = "IDF_WIrelessTAB";
const char* STA_PASS = "cikupawifi123";

// Endpoint PHP yang menerima history. Sesuaikan IP/path kalau server berubah.
const char* HISTORY_SERVER_URL = "http://10.126.48.208/api/history_receive.php";
// Kunci sederhana biar endpoint tidak bisa diakses sembarangan di jaringan pabrik.
const char* API_KEY = "ganti-dengan-kunci-rahasia-anda";

// Identitas device ini (kalau nanti ada lebih dari 1 unit fingerprint di pabrik)
const char* DEVICE_ID = "FP-GATE-01";
...
// ---------- Riwayat scan (RAM only, max 20 terakhir, untuk tampilan lokal) ----------
...
// ---------- Queue & task untuk kirim history ke server pabrik (non-blocking) ----------
struct HistoryPacket {
  int id;
  char name[32];
  bool success;
  unsigned long ts; // millis() device saat kejadian (informasi tambahan, bukan waktu absolut)
};
...
void networkTask(void* param) {
  ...
  for (;;) {
    // Kelola reconnect STA sendiri, tanpa delay() panjang yang mengganggu core lain
    ...
    // Tunggu data history masuk queue (maksimal 500ms), tidak memblokir core lain
    if (xQueueReceive(historyQueue, &pkt, pdMS_TO_TICKS(500)) == pdTRUE) {
      if (WiFi.status() == WL_CONNECTED) {
        HTTPClient http;
        http.setConnectTimeout(2000); // maksimal 2 detik nyoba konek
        http.setTimeout(3000);        // maksimal 3 detik nunggu response
        http.begin(HISTORY_SERVER_URL);
        http.addHeader("Content-Type", "application/json");
        http.addHeader("X-API-KEY", API_KEY);

        String safeName = String(pkt.name);
        safeName.replace("\"", "'"); // hindari JSON rusak kalau nama ada tanda kutip

        String json = "{";
        json += "\"device_id\":\"" + String(DEVICE_ID) + "\",";
        json += "\"finger_id\":" + String(pkt.id) + ",";
        json += "\"name\":\"" + safeName + "\",";
        json += "\"success\":" + String(pkt.success ? "true" : "false") + ",";
        json += "\"device_ts\":" + String(pkt.ts);
        json += "}";

        int code = http.POST(json);
        Serial.printf("[NET] Kirim history ID %d -> HTTP %d\n", pkt.id, code);
        http.end();
      } else {
        Serial.println("[NET] WiFi pabrik belum konek, history ini di-skip (tidak diretry).");
      }
    }
  }
}
...
void addHistory(int id, String name, bool success) {
  // Simpan ke RAM untuk tampilan lokal di web AP
  ...
  // Dorong ke queue supaya dikirim task Core 0 ke server pabrik.
  // Non-blocking: kalau queue penuh, data ini di-skip, sensor tetap jalan normal.
  HistoryPacket pkt;
  pkt.id = id;
  name.toCharArray(pkt.name, sizeof(pkt.name));
  pkt.success = success;
  pkt.ts = millis();
  if (xQueueSend(historyQueue, &pkt, 0) != pdTRUE) {
    Serial.println("[NET] Queue history penuh, data ini di-skip.");
  }
}
...
<style>
  *{box-sizing:border-box;}
  ...
  .netdot{width:7px;height:7px;border-radius:50%;display:inline-block;margin-right:5px;background:#cbd5e1;}
  .netdot.on{background:#22c55e;}
  .netlabel{font-size:11px;color:#94a3b8;display:flex;align-items:center;margin-right:10px;}
...
<body>
  <div class="topbar">
    ...
    <div style="display:flex;align-items:center;">
      <span class="netlabel"><span class="netdot" id="netdot"></span><span id="netlabelText">Cek jaringan...</span></span>
  ...
  <div class="wrap">
    ...
    <div id="crud" class="panel">
      ...
      <button class="btn-secondary" onclick="loadList()">â†» Refresh Daftar</button>
      <div id="list"><div class="empty">Klik refresh untuk lihat data...</div></div>
    </div>

    <div id="history" class="panel">
      <h2>Riwayat Scan (20 terakhir)</h2>
      <button class="btn-secondary" onclick="loadHistory()">â†» Refresh Riwayat</button>
...
function loadNetStatus(){
  fetch('/netstatus').then(r => r.json()).then(data => {
    const dot = document.getElementById('netdot');
    const label = document.getElementById('netlabelText');
    if(data.connected){
      dot.classList.add('on');
      label.innerText = 'Server pabrik OK';
    } else {
      dot.classList.remove('on');
      label.innerText = 'Server pabrik offline';
    }
  });
}

setInterval(pollStatus, 1500);
setInterval(loadNetStatus, 4000);
loadNetStatus();
</script>
</body>
</html>
...
void handleHistory() {
  ...
  for (int i = 0; i < historyCount; i++) {
    ...
    if (i > 0) json += ",";
    String label = "ID " + String(history[idx].id) + " â€” " + history[idx].name;
...
void handleNetStatus() {
  if (!requireLogin()) return;
  String json = "{\"connected\":" + String(staConnected ? "true" : "false") + "}";
  server.send(200, "application/json", json);
}
...
void setup() {
  ...
  // AP tetap aktif untuk akses admin lokal, sekaligus konek ke WiFi pabrik (STA)
  WiFi.mode(WIFI_AP_STA);
  WiFi.softAP(AP_SSID, AP_PASS);
  Serial.print("AP IP address: ");
  Serial.println(WiFi.softAPIP());

  // Queue + task khusus kirim history, jalan di Core 0 supaya tidak
  // pernah mengganggu polling sensor & web server AP yang jalan di Core 1 (loop()).
  historyQueue = xQueueCreate(20, sizeof(HistoryPacket));
